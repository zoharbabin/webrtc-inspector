...
For startup/protocol timing (when did the offer go out, when did ICE connect, when did a track unmute), don't hand-roll a `WebSocket`/`RTCPeerConnection` monkey-patch through the automation tool's eval — by the time that eval runs, the page's own scripts may have already grabbed the original constructor, so the patch silently intercepts nothing. `captureEvents()` doesn't have this race: the extension's content script runs at `document_start` in the MAIN world, before the page's own scripts, so it already has every `pc-created`/`ice-candidate-*`/`websocket-*`/`track-*` event with real timestamps from before you ran a single eval call.
